/// @DnDAction : YoYo Games.Common.If_Variable
/// @DnDVersion : 1
/// @DnDHash : 0CDD4EDE
/// @DnDArgument : "var" "room"
/// @DnDArgument : "value" "DoorTest"
if(room == DoorTest)
{
	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 30FCB7ED
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_heavybreathing"
	/// @DnDSaveInfo : "soundid" "sou_heavybreathing"
	audio_stop_sound(sou_heavybreathing);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 5331D76B
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_malegasp"
	/// @DnDSaveInfo : "soundid" "sou_malegasp"
	audio_stop_sound(sou_malegasp);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 30DA1E4B
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_shot"
	/// @DnDSaveInfo : "soundid" "sou_shot"
	audio_stop_sound(sou_shot);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 3EA53776
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_tvhum"
	/// @DnDSaveInfo : "soundid" "sou_tvhum"
	audio_stop_sound(sou_tvhum);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 2F20065E
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_slotjackpot"
	/// @DnDSaveInfo : "soundid" "sou_slotjackpot"
	audio_stop_sound(sou_slotjackpot);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 09B5A1F6
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_slotlose"
	/// @DnDSaveInfo : "soundid" "sou_slotlose"
	audio_stop_sound(sou_slotlose);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 25B65951
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_slotmachine"
	/// @DnDSaveInfo : "soundid" "sou_slotmachine"
	audio_stop_sound(sou_slotmachine);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 067251F1
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_slotstop"
	/// @DnDSaveInfo : "soundid" "sou_slotstop"
	audio_stop_sound(sou_slotstop);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 38C1929D
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_xp_error"
	/// @DnDSaveInfo : "soundid" "sou_xp_error"
	audio_stop_sound(sou_xp_error);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 46D622C0
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_coincollect"
	/// @DnDSaveInfo : "soundid" "sou_coincollect"
	audio_stop_sound(sou_coincollect);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 512F4E41
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_damage"
	/// @DnDSaveInfo : "soundid" "sou_damage"
	audio_stop_sound(sou_damage);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 1DF2B425
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_gameend"
	/// @DnDSaveInfo : "soundid" "sou_gameend"
	audio_stop_sound(sou_gameend);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 5F79DD30
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_waytogo"
	/// @DnDSaveInfo : "soundid" "sou_waytogo"
	audio_stop_sound(sou_waytogo);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 504DFC75
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_fartreverb"
	/// @DnDSaveInfo : "soundid" "sou_fartreverb"
	audio_stop_sound(sou_fartreverb);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 18AF277A
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_fireworkboom"
	/// @DnDSaveInfo : "soundid" "sou_fireworkboom"
	audio_stop_sound(sou_fireworkboom);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 2CB9F302
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_fireworklaunch"
	/// @DnDSaveInfo : "soundid" "sou_fireworklaunch"
	audio_stop_sound(sou_fireworklaunch);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 3B93FBAA
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_angels"
	/// @DnDSaveInfo : "soundid" "sou_angels"
	audio_stop_sound(sou_angels);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 0A8BCFFD
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_flashon"
	/// @DnDSaveInfo : "soundid" "sou_flashon"
	audio_stop_sound(sou_flashon);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 716E9C8A
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_ghostdefeat"
	/// @DnDSaveInfo : "soundid" "sou_ghostdefeat"
	audio_stop_sound(sou_ghostdefeat);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 1A560E10
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_ghostgrowl"
	/// @DnDSaveInfo : "soundid" "sou_ghostgrowl"
	audio_stop_sound(sou_ghostgrowl);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 6FE4FDBC
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_holychant"
	/// @DnDSaveInfo : "soundid" "sou_holychant"
	audio_stop_sound(sou_holychant);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 202F8BBB
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_spookyhouse"
	/// @DnDSaveInfo : "soundid" "sou_spookyhouse"
	audio_stop_sound(sou_spookyhouse);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 13C820D4
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_animewow"
	/// @DnDSaveInfo : "soundid" "sou_animewow"
	audio_stop_sound(sou_animewow);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 31981395
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_glassbreak"
	/// @DnDSaveInfo : "soundid" "sou_glassbreak"
	audio_stop_sound(sou_glassbreak);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 04E39343
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_kiss"
	/// @DnDSaveInfo : "soundid" "sou_kiss"
	audio_stop_sound(sou_kiss);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 1F1CD36F
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_movecursor"
	/// @DnDSaveInfo : "soundid" "sou_movecursor"
	audio_stop_sound(sou_movecursor);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 22451B21
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_selection"
	/// @DnDSaveInfo : "soundid" "sou_selection"
	audio_stop_sound(sou_selection);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 73469E5E
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_shock"
	/// @DnDSaveInfo : "soundid" "sou_shock"
	audio_stop_sound(sou_shock);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 0BCD1C63
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_conga"
	/// @DnDSaveInfo : "soundid" "sou_conga"
	audio_stop_sound(sou_conga);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 0920FEAA
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_recordscratch"
	/// @DnDSaveInfo : "soundid" "sou_recordscratch"
	audio_stop_sound(sou_recordscratch);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 0F696055
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_rockcrush"
	/// @DnDSaveInfo : "soundid" "sou_rockcrush"
	audio_stop_sound(sou_rockcrush);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 0BF92FD2
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_ropecut"
	/// @DnDSaveInfo : "soundid" "sou_ropecut"
	audio_stop_sound(sou_ropecut);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 69031079
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_bloodsquirt"
	/// @DnDSaveInfo : "soundid" "sou_bloodsquirt"
	audio_stop_sound(sou_bloodsquirt);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 49E191CD
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_boingjump"
	/// @DnDSaveInfo : "soundid" "sou_boingjump"
	audio_stop_sound(sou_boingjump);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 44D31474
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_goombadeath"
	/// @DnDSaveInfo : "soundid" "sou_goombadeath"
	audio_stop_sound(sou_goombadeath);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 658CAA62
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_harp"
	/// @DnDSaveInfo : "soundid" "sou_harp"
	audio_stop_sound(sou_harp);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 16694D0D
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_yeehaw"
	/// @DnDSaveInfo : "soundid" "sou_yeehaw"
	audio_stop_sound(sou_yeehaw);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 6EF49B5B
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_projector"
	/// @DnDSaveInfo : "soundid" "sou_projector"
	audio_stop_sound(sou_projector);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 00FCAB6C
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_biglaugh"
	/// @DnDSaveInfo : "soundid" "sou_biglaugh"
	audio_stop_sound(sou_biglaugh);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 6EB7A193
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_crowdboo"
	/// @DnDSaveInfo : "soundid" "sou_crowdboo"
	audio_stop_sound(sou_crowdboo);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 6FDB2582
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_jumpscare"
	/// @DnDSaveInfo : "soundid" "sou_jumpscare"
	audio_stop_sound(sou_jumpscare);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 311F7067
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_robopopo"
	/// @DnDSaveInfo : "soundid" "sou_robopopo"
	audio_stop_sound(sou_robopopo);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 52C1B310
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_shittybackground"
	/// @DnDSaveInfo : "soundid" "sou_shittybackground"
	audio_stop_sound(sou_shittybackground);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 2B9CB390
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_spank"
	/// @DnDSaveInfo : "soundid" "sou_spank"
	audio_stop_sound(sou_spank);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 1F2526F5
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_clockring"
	/// @DnDSaveInfo : "soundid" "sou_clockring"
	audio_stop_sound(sou_clockring);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 17F54BE3
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_failclock"
	/// @DnDSaveInfo : "soundid" "sou_failclock"
	audio_stop_sound(sou_failclock);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 12FCC7AC
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_whoosh"
	/// @DnDSaveInfo : "soundid" "sou_whoosh"
	audio_stop_sound(sou_whoosh);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 5268EB0C
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_winclock"
	/// @DnDSaveInfo : "soundid" "sou_winclock"
	audio_stop_sound(sou_winclock);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 439273E3
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_correct"
	/// @DnDSaveInfo : "soundid" "sou_correct"
	audio_stop_sound(sou_correct);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 587AAA90
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_incorrect"
	/// @DnDSaveInfo : "soundid" "sou_incorrect"
	audio_stop_sound(sou_incorrect);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 2A16EAFD
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_slyshock"
	/// @DnDSaveInfo : "soundid" "sou_slyshock"
	audio_stop_sound(sou_slyshock);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 6ABC4748
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_electricshock"
	/// @DnDSaveInfo : "soundid" "sou_electricshock"
	audio_stop_sound(sou_electricshock);

	/// @DnDAction : YoYo Games.Audio.Stop_Audio
	/// @DnDVersion : 1
	/// @DnDHash : 7DE6B022
	/// @DnDParent : 0CDD4EDE
	/// @DnDArgument : "soundid" "sou_donkey"
	/// @DnDSaveInfo : "soundid" "sou_donkey"
	audio_stop_sound(sou_donkey);
}